...
export async function extractFileContentFromSource(params: {
  ...
  if (mimeType === "application/pdf") {
    ...
    if (remoteConfig.apiUrl) {
      try {
        const remoteExtracted = await extractPdfContentRemote({
          buffer,
          filename: source.filename,
          limits,
          config: remoteConfig,
        });
        const text = remoteExtracted.text ? clampText(remoteExtracted.text, limits.maxChars) : "";
        return {
          filename,
          text,
          images: remoteExtracted.images.length > 0 ? remoteExtracted.images : undefined,
        };
      } catch (err) {
